...
A complete solution for handling text is surprisingly complex. Although perhaps this should not be so surprising when we consider that a 
...
A key decision that anyone wanting to render text needs to make is whether to use the platform's built-in libraries where available (DirectWrite on windows, CoreText on macOS/iOS) or whether to use open source libraries (which are the only option on Linux/Android).
