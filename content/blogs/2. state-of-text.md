...
* **Rasterisation** converts the scaled and hinted vector outlines into a raster bitmap of pixels. Scaling and hinting are sometimes treated as part of "rasterisation" but in this article we will treat these as separate processes.
...
There are only two solutions for scaling/hinting: Swash and Skrifa. Furthermore, these solutions are related: Skrifa is an improved and extended version of Swash (partially) by the same author, with Swash in . Swash is effectively in maintenance mode with future development work going in Skrifa, and does not represent a good long-term solution unless that situation changes.

We may also notice that Skrifa is dependent on read-fonts. Thus if we wish to 



Both stacks above can use Swash. Cosmic-text cannot yet

- Swash was the original library and contains Skrifa contains a productionised version of Swashes
...
ttf_parser and read-fonts are both excellent libraries with little separate them at this point. 
