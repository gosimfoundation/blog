...
## Key Stakeholders in the Rust text ecosystem
...
[Typst](https://github.com/typst/typst) is a new typesetting similar to LaTeX. They have their own layout library which isn't really generally usable, however some of their developers (notably Laurenz Stampfl ([@LaurenzV](https://github.com/LaurenzV))) have been contributing to the resvg stack of crates.
...
The Google Fonts organisation within Google (who share a number of members with Linebender) and have a number of very exciting projects in the Rust text space under the [Fontations](https://github.com/googlefonts/fontations) project. They are currently making strong progress on [Skrifa](https://github.com/googlefonts/fontations/tree/main/skrifa), a pure Rust parser/scaling/hinter, which is already shipping in Chrome Canary and is actively being pursued as a complete replacement for FreeType in Chrome.
...
- The [ab_glyph](https://github.com/alexheretic/ab-glyph) and [glyph-brush](https://github.com/alexheretic/glyph-brush) set of crates are 
- fontdue
- Makepad
- Bevy
- Skia
...
Unless your needs are very simple (no non-latin language support), then the ab_glyph stack isn't going to be suitable. So we can dismiss that from our analysis. 

- A servo-style system C-library based stack.

That leaves 2 choices:

- The cosmic-text stack (cosmic-text + rustybuzz + ttf_parser + fontdb + swash(scaling/hinting))
- The parley stack (parley + swash(shaping) + skrifa + fontique + swash(scaling/hinting))
